  ...
  kids=[
    StringFileInfo(
      ...
      StringTable(
        ...
        StringStruct(u'FileDescription', u'Turbo Tunnel'),
        StringStruct(u'FileVersion', u'%(main_ver)d.%(sub_ver)d.%(min_ver)d'),
        StringStruct(u'InternalName', u'turbo-tunnel.exe'),
        StringStruct(u'LegalCopyright', u'Copyright (C) 2017-%(year)d drunkdream.com. All Rights Reserved'),
        StringStruct(u'OriginalFilename', u'turbo-tunnel.exe'),
        StringStruct(u'ProductName', u'turbo-tunnel'),
    ...
    if not os.path.isfile(main_file):
        with open(main_file, "w") as fp:
            fp.write(
                r"""# -*- coding: utf-8 -*-
import sys
from turbo_tunnel.__main__ import main
...
"""
    ...
    if sys.platform == "win32":
        ...
        with open(version_file, "w") as fp:
            fp.write(text)
        cmdline = "python -m PyInstaller -F -c %s -n turbo-tunnel --version-file %s" % (
            main_file,
            version_file,
        )
    else:
        cmdline = "python -m PyInstaller -F -w %s -n turbo-tunnel" % main_file
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="build-turbo-tunnel", description="Build turbo-tunnel tool."
    )
    parser.add_argument(
        "--backend",
        help="build backend",
